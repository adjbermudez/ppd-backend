...
from api.models import User, Unore, News, Actualizacion, Banner,Innovation, db
...
@api.route("/innovation", methods=["GET"])
def get_innovations():
  if request.method == "GET":
    innovations = Innovation.get_all()
    if innovations is None:
      return jsonify({'message':'Error try again later'}), 500
    return jsonify([innovation.serialize() for innovation in innovations]), 200
  return jsonify({'message':'method not allowed'}),405


@api.route("/innovation", methods=["POST"])
@jwt_required()
def create_innovation():
  if request.method == "POST":
    ...
    data ={
      'title':data_form.get('description'),
      'url_file':data_form.get('nameFile'),
      'url_imgage':data_files.get('innovationImage'),
    }
    ...
    if data.get('title') is None:
      return jsonify({'message':'Bad request'}), 400
    if data.get('url_file') is None:
      return jsonify({'message':'Bad request'}), 400
    if data.get('url_imgage') is None:
      return jsonify({'message':'Bad request'}), 400
    
    response_img = uploader.upload(data.get('url_imgage'))
    data.update({'url_imgage':response_img['secure_url']})
    data.update({'public_id':response_img['public_id']})

    innovation = Innovation.create(data)
    if type(innovation) == Innovation:
      return innovation.serialize(), 201
...
@api.route("/innovation/<int:id>", methods=["DELETE"])
@jwt_required()
def delete_innovation (id=None):
  if request.method == "DELETE":
    if id is None:
      return jsonify({'message':'Bad request'}), 400
    ...
    if innovation is None:
      return jsonify({'message':'Innovation not found'}), 404
    
    try:
      db.session.delete(innovation)
      db.session.commit()
      uploader.destroy(innovation.public_id)
      return jsonify({'message':'Innovation deleted'}), 204
    except Exception as error:
      print(error.args)
      return jsonify({'message':'Error try again later'}), 500
  return jsonify({'message':'method not allowed'}),405